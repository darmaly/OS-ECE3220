...
// returns TRUE if file is open, FALSE if invalid/closed
...
// returns FALSE if length too long / non-alphanumeric
...
// return file descriptor index or 0 if failed 
...
// returns file descriptor index if it is in the director, else 0
unsigned int btrfs_map_name_to_fd( char *name ){
  unsigned int fd;
  if( !btrfs_check_valid_name( name ) ) return( 0 );
...
void btrfs_init(){
  ...
  file_allocation_table = (unsigned char *) &storage[sizeof(struct directory_entry) * N_DIRECTORY_ENTRIES];
  for( i = 0; i < N_BYTES; i++ ){
    storage[i] = 0;
  }
}
...
unsigned int btrfs_exists( char *name ){
  if( !btrfs_check_valid_name( name ) ) return( FALSE );
  if( btrfs_map_name_to_fd( name ) == 0 ) return( FALSE );
  return( TRUE );
}
...
unsigned int btrfs_create( char *name ){
  ...
  if( !btrfs_check_valid_name( name ) ) return( 0 );
  if( btrfs_map_name_to_fd( name ) != 0 ) return( 0 );
  file_descriptor = btrfs_new_directory_entry();
...
unsigned int btrfs_open( char *name ){
  unsigned int file_descriptor;
  if( !btrfs_check_valid_name( name ) ) return( 0 );
  file_descriptor = btrfs_map_name_to_fd( name );
  if( file_descriptor == 0 ) return( 0 );
...
unsigned int btrfs_size( unsigned int file_descriptor ){
  if( !btrfs_check_fd_in_range( file_descriptor ) ) return( MAX_FILE_SIZE + 1 );
