...
int main (int argc, char **argv) {
    ...
    /* ROUND ROBIN */
        ...
        while (list->task_counter != 0) {
            ...
            while (rover != NULL && rover->arrival_time == time) {
                linkedRemove(list);
                if (rQueue->head != NULL && rQueue->head->arrival_time != rover->arrival_time && rQueue->head == rQueue->tail) {
                    linkedInsert(rQueue, emptyJob, rover, time, -4);
                } 
                else if (rQueue->head != NULL && rQueue->head != rQueue->tail) {
                    linkedInsert(rQueue, emptyJob, rover, time, -5);
                } 
                else {
